...
# 二叉树的表示
說了那么多，那么怎么表示一棵二叉树呢？其實你發现會和鏈表有一些相似之处，一個節點，然後節點需要保存孩子的指針，我以构造下邊這個二叉树為例子：
我們先定義一個类表示節點：
...
# 二叉树的遍历
不知道你有没有發现，二叉树其實是一種遞迴結構，因為單独拿出來一個 subtree 子树出來，其實它還是一棵树。那遍历它就很方便啦，我們可以直接用遞迴的方式來遍历它。但是當处理顺序不同的时候，树又分為三種遍历方式:
