...
def fib(n):
    if n <= 1:  # 0 or 1
        return n
    return f(n - 1) + f(n - 2)  # 由于涉及到重复计算，這個遞迴函数在 n 大了以後會非常慢。 O(2^n)
...
當容量满了以後，我們踢出最早訪問的元素。假如用一個鏈表來表示的话：

[1] -> [2] -> [3]

假设最後面是最後訪問的，當訪問到一個元素以後，我們把它放到最後。當容量满了，我們踢出第一個元素就行了。
一開始的想法可能是用一個鏈表來记录訪問顺序，但是單鏈表有個問題就是如果訪問了中間一個元素，我們需要拿掉它并且放到鏈表尾部。
而單鏈表无法在O(1)的時間内删除一個節點（必须要先搜索到它），但是双端鏈表可以，因為一個節點记录了它的前後節點，
只需要把要删除的節點的前後節點鏈接起來就行了。
還有個問題是如何把删除後的節點放到鏈表尾部，如果是循環双端鏈表就可以啦，我們有個 root 節點鏈接了首位節點，
只需要让 root 的前一個指向這個被删除節點，然後让之前的最後一個節點也指向它就行了。

使用了循環双端鏈表之後，我們的操作就都是 O(1) 的了。這也就是使用一個 dict 和一個 循環双端鏈表 實現LRU 的思路。
不過一般我們使用内置的 OrderedDict(原理和這個类似)就好了，要實現一個循環双端鏈表是一個不簡單的事情。
