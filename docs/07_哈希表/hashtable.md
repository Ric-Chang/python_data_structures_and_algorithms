# 哈希表
不知道你有没有好奇過為什麼 Python 里的 dict 和 set 查找速度這麼快呢，用了什麼黑魔法嗎？
經常听别人說哈希表(也叫做散列表)，究竟什麼是哈希表呢？這一章我們來介绍哈希表，後续章節我們會看到 Python 中的字典和集合是如何實現的。

# 哈希表的工作過程
前面我們已經講到了数組和鏈表，数組能通過下標 O(1) 訪問，但是删除一個中間元素却要移動其他元素，時間 O(n)。
循環双端鏈表倒是可以在知道一個節點的情况下迅速删除它，但是吧查找又成了 O(n)。

难道就没有一種方法可以快速定位和删除元素嗎？似乎想要快速找到一個元素除了知道下標之外别无他法，于是乎聪明的计算機科學家又想到了一種方法。
能不能给每個元素一種『逻辑下標』，然後直接找到它呢，哈希表就是這種實現。它通過一個哈希函数來计算一個元素应该放在数組哪個位置，當然對于一個
特定的元素，哈希函数每次计算的下標必须要一样才可以，而且范围不能超過给定的数組长度。

我們還是以书中的例子說明，假如我們有一個数組 T，包含 M=13 個元素，我們可以定義一個簡單的哈希函数 h

```
h(key) = key % M
```

這裡取模運算使得 h(key) 的結果不會超過数組的长度下標。我們來分别插入以下元素：
...
# 哈希冲突 (collision)
這裡到插入 226 這個元素的时候，不幸地發现 h(226) = h(96) = 5，不同的 key 通過我們的哈希函数计算後得到的下標一样，
這種情况成為哈希冲突。怎麼办呢？聪明的计算機科學家又想到了办法，其實一種直觀的想法是如果冲突了我能不能让数組中
對应的槽变成一個鏈式結構呢？這就是其中一種解决方法，叫做 **鏈接法(chaining)**。如果我們用鏈接法來處理冲突，後面的插入是這样的：

![](./insert_hash_chaining.png)

這样就用鏈表解决了冲突問題，但是如果哈希函数選不好的话，可能就导致冲突太多一個鏈变得太长，這样查找就不再是 O(1) 的了。
還有一種叫做開放寻址法(open  addressing)，它的基本思想是當一個槽被占用的时候，采用一種方式來寻找下一個可用的槽。
（這裡槽指的是数組中的一個位置），根據找下一個槽的方式不同，分為：
...
我們選一個簡單的二次探查函数 $ h(k, i) = (home + i^2) \% m $，它的意思是如果
遇到了冲突，我們就在原始计算的位置不断加上 i 的平方。我写了段代碼來模擬整個计算下標的過程：
...
# 哈希函数
到這裡你应该明白哈希表插入的工作原理了，不過有個重要的問題之前没提到，就是 hash 函数怎麼選？
當然是散列得到的冲突越來越小就好啦，也就是說每個 key 都能尽量被等可能地散列到 m 個槽中的任何一個，并且与其他 key 被散列到哪個槽位无關。
如果你感興趣，可以閱讀後面提到的一些参考資料。影片里我們使用二次探查函数，它相比線性探查得到的結果冲突會更少。


# 装载因子(load factor)
如果继续往我們的哈希表里塞東西會發生什麼？空間不够用。這裡我們定義一個负载因子的概念(load factor)，其實很簡單，就是已經使用的槽数比哈希表大小。
...
# 重哈希(Rehashing)
當负载因子超過 0.8 的时候，需要进行 rehashing 操作了。步骤就是重新開辟一塊新的空間，開多大呢？感興趣的话可以看下 cpython 的 dictobject.c 文件然後搜索
...
# HashTable ADT
實践是检验真理的唯一標准，這裡我們來實現一個简化版的哈希表 ADT，主要是為了让你更好地了解它的工作原理，有了它，後面實現起 dict 和 set 來就小菜一碟了。
這裡我們使用到了定长数組，還记得我們在数組和列表章節里實現的 Array 吧，這裡要用上了。

解决冲突我們使用二次探查法，模擬 cpython 二次探查函数的實現。我們來實現三個哈希表最常用的基本操作，這實際上也是使用字典的时候最常用的操作。
...
- Slot 在二次探查法里為什麼不能直接删除？為什麼我們要给它定義几個状态？

# 延伸閱讀
- 《Data Structures and Algorithms in Python》11 章 Hash Tables
- 《算法导论》第三版 11 章散列表，了解几種哈希冲突的解决方式，以及為什麼我們選择二次探查而不是線性探查法？
