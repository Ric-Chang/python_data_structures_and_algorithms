...
# 哈希冲突 (collision)
這裡到插入 226 這個元素的时候，不幸地发现 h(226) = h(96) = 5，不同的 key 通過我們的哈希函数计算後得到的下标一样，
這種情况成為哈希冲突。怎么办呢？聪明的计算机科学家又想到了办法，其实一種直觀的想法是如果冲突了我能不能让数组中
...
for number in to_insert:
    ...
    first_index = index
    i = 1
    while index in inserted_index_set:   # 如果计算发现已經占用，继续计算得到下一個可用槽的位置
...
# 装载因子(load factor)
如果继续往我們的哈希表里塞東西會发生什么？空間不够用。這裡我們定義一個负载因子的概念(load factor)，其实很簡單，就是已經使用的槽数比哈希表大小。
...
GROWTH_RATE 這個關键字，你會发现不同版本的 cpython 使用了不同的策略。python3.3 的策略是扩大為已經使用的槽数目的兩倍。開辟了新空間以後，會把原來哈希表里
不為空槽的數據重新插入到新的哈希表里，插入方式和之前一样。這就是 rehashing 操作。

# HashTable ADT
实践是检验真理的唯一标准，這裡我們來實現一個简化版的哈希表 ADT，主要是為了让你更好地了解它的工作原理，有了它，後面實現起 dict 和 set 來就小菜一碟了。
這裡我們使用到了定长数组，還记得我們在数组和列表章節里實現的 Array 吧，這裡要用上了。

解决冲突我們使用二次探查法，模擬 cpython 二次探查函数的實現。我們來實現三個哈希表最常用的基本操作，這实际上也是使用字典的时候最常用的操作。
