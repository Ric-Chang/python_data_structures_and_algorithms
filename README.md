...
## 課程簡介
數據結構和算法是每個程序員需要掌握的基礎知識之一，也是面試中跨不過的槛。目前關於 Python 算法和數據結構的系统中文资料比較欠缺，
筆者嘗試錄制影片教程帮助 Python 開發者掌握常用算法和數據結構，提升開發技能。
本教程是付费教程(文字内容和原始碼免费)，因為筆者錄制的過程中除了購買軟體、手寫板等硬體之外，業餘需要花费很多時間和精力來錄制影片、查资料、編寫課件和原始碼，養家糊口不容易，希望大家體谅。
...
- 很多自學 Python 的工程師對基礎不够重視，面試也發現很多數據結構和算法不過關，很多人挂在了基礎的數據結構和算法上
- 缺少工程应用場景下的講解，很多講算法的资料太『教科書化』。本書實現的原始碼工程上可用
- 網上很多影片教程不够循序漸進，不成系统
...
也可以作為回顾。課程儘量用通俗的方式講解，結合 python 語言和日常開發實践的經驗。書中原始碼可以作為大家的面試筆試参考。
...
## 目錄結構
這裡講解的章節我参考了下邊教材中列舉的一些書籍，並且自己設計了大綱，争取做到循序漸進，簡單實用。因為實現一些高级數據結構的时候會用到
很多底層數據結構，防止跳跃太大導致讀者理解困難。

課程的目錄結構如下，每一章都有配套的文字講義(markdown)，示例原始碼，影片講解，詳細的講解一般會放在影片里，使用手寫板來
...
## 編程語言
我們這裡使用最近很火的Python。Python 入門簡單而且是個多面手，在爬虫、web 後端、運维、數據分析、AI、量化投资等領域都有 Python 的身影，
...
Python 抽象程度比較高， 我們能用更少的原始碼來實現功能，同时不用像 C/C++ 那樣担心内存管理、指針操作等底層問題，
把主要心思放在算法邏輯本身而不是語言細節上，Python 也号称伪原始碼語言。所有原始碼示例使用 Python2/3 兼容原始碼，
...
**請注意: 本教程不是零基礎教程，著重於使用 Python 實現常用算法和數據結構，不適合從來没有學過算法和數據結構的新手同學，購買之前請慎重考虑，請确保你之前看過一本數據結構和算法的教材，最好有過其他語言實現算法的經驗**

# 預備知識
（注意：有些同學看起來很吃力，為了不花冤枉钱，我建議你先整體瀏覽本電子書的内容和原始碼是否在自己的理解範圍内，再决定是否購買影片。有些概念不是立马就能理解的，需要反复思考實践）

- 了解基本的數據結構和算法的概念，不適合**完全**没有了解過算法的新手，更不適合 Python 基礎都没掌握的同學。購買之前請慎重考虑
- 無需太多數學基礎，仅在算法時間複雜度分析的时候會用到一些簡單數學知識。對於學習基礎算法，邏輯思维可能更重要一些
...
[《算法導論》第三版]( https://book.douban.com/subject/20432061/): 喜欢數學证明和板砖書的同學可以参考，有很多高级主题。使用伪原始碼

## 算法可視化

學習算法的過程中有时候會比較抽象，這裡给大家推薦一些可視化的網站，方便更直觀地理解：
...
- 每個算法和數據結構都有講義、影片(包含講解、圖示、手動模擬)、源原始碼。其中只有影片内容為付费内容
- 講義循序漸進，結合自己的學習和使用經驗講解。github 上實时更新
- 影片演示更加直觀易懂
- 演示原始碼實現思路，所有原始碼在影片里均現場編寫
- 偏向工程应用和原始碼實現。原始碼直接可以用。每個文件都是自包含的，你可以直接運行和调試，這是目前大部分書籍做得不到位的地方
- 良好的工程實践：[編碼之前碎碎念(工程實践)](http://python-web-guide.readthedocs.io/zh/latest/codingstyle/codingstyle.html)。
這是很多看了几本書没有太多业界實践經驗就敢講課的培训班老師教不了的。**知識廉價，經驗無價**
- 每個實現都會有單測來驗证，培養良好的編碼和測試习惯，傳授工程經驗
- 結合 cpython 底層實現講解（比如list 内存分配策略等），避免一些使用上的坑。並且會用 python 來模擬内置 dict 等的實現
- 每篇講義後有思考题和延伸閱讀鏈接，帮助大家加深思考和理解。大部分题目答案都可以網路上搜索到
...
- 延伸閱讀。我會附上一些閱讀资料方便想深入學習的同學

## 如何获取每章原始碼

注意每一章目錄里都有 py 文件，在電子書里看不到。clone 下本原始碼仓库找到對应目錄里的 python 文件即是每章涉及到的原始碼。
由於原始碼實現千差万别，本書原始碼實現具有一定的個人風格，不代表最佳實現，仅供参考。


## 如何學習
筆者講課錄制影片的過程也是自己再整理和學習的過程，錄制影片之前需要参考很多资料
希望對所講到的内容，你能够
...
- 理解所講的每個數據結構和算法的
    ...
    - 時間、空間複雜度
    - 使用場景，什麼时候用
- 自己嘗試實現，如果抛開影片自己寫起來有困難可以反复多看几次影片，一定要自己手動實現。很多面試可能會讓手寫。一次不行就看完原理後多實践几次，直到能自己独立完成。
- 每章講義後面都會有我設計的几個小問題，最好能够回答上來。同时還有原始碼练习题，你可以挑戰下自己的掌握程度。
- 最好按照顺序循序漸進，每章都會有铺垫和联系，後面的章節可能會使用到前面提到的數據結構
- 根據自己的基礎結合我列舉的教材和影片學習，第一次理解不了的可以反复多看几次，多編寫原始碼练习到熟练為止
...
推薦使用以下工具進行開發，如果使用編辑器最好装對 应 Python 插件，筆者影片演示中使用了 vim，讀者可以自己挑選自己喜欢的開發工具：
...
```
將以上内容放到 test.sh 文件後加上可執行权限, `chmod +x test.sh`，之後就可以用

```
'./test.sh somefile.py'
```
每次我們改動了原始碼，就會自動執行原始碼里的單元測試了。pytest 會自動發現以 test
開頭的函数並執行測試原始碼。良好的工程需要我們用單測來保证，將來即使修改了内部實現邏輯也方便做回归驗证。

或者你可以在的 ~/.bashrc  or  ~/.zshrc 里邊加上這個映射（别忘記加上之後source下）:
...
然後在你的原始碼目錄里頭執行 `watchtest pytest -s somefile.py` 一樣的效果


## 測試用例設計

筆者在刚學習編程的时候总是忘記處理一些特例(尤其是動态語言可以傳各種值)，為了養成良好的編程和測試习惯，在編寫單元測試用例的时候，
我們注意考虑下如下測試用例(等價類划分)：
...
當然我們也不用做的非常細致，要不然寫測試是一件非常繁琐累人的事情，甚至有时候為了測試而測試，只是為了讓單測覆盖率好看點。
當然如果是web应用用户輸入，我們要假設所有的参数都是不可信的。 但是很多内部调用的函数我們基於约定來編程，如果你瞎傳参数，那就是调用者的責任了。


## 勘誤

輸出其實也是一種再學習的過程，中途需要查看大量资料、編寫講義、影片錄制、原始碼編寫等，難免有疏漏甚至錯誤之處。
有出版社找過筆者想讓我出書，一來自己對出書興趣不大，另外感覺書籍相對影片不够直觀，有錯誤也不能及时修改，打算直接把所有文字内容講義和原始碼等放到 github 上，供大家免费查阅。

如果你發現文字内容、原始碼内容、影片内容有錯誤或者有疑問，欢迎在 github 上提 issue 討論(或者網易公開課评論區)，或者直接提 Merge Request，我會儘量及时修正相關内容，防止對讀者产生誤導。
同时非常感谢认真學習並及时發現書中錯誤的同學，非常欢迎針對知識本身的交流和討論，任何建議和修正我都會认真求证。
對於提出修正意見或者提交原始碼的同學，由於人数比較多這裡就不一一列舉了，可以在以下列表查看，再次感谢你们。筆者信奉開源精神，『眼睛足够多，bug 無處藏』。
如果您發現影片中的原始碼有誤，請及时使用 git pull 拉取本项目的原始碼更新，最好用目前最新的原始碼來學習和實践。
...
- 優先在網易云課堂的討論區提問，方便别的同學瀏覽。如果未購買影片，也可以直接在 github 里提出 issue，筆者有空會给大家解答和討論。
- 描述儘量具體，影片或者原始碼哪一部分有問題？請儘量把涉及章節和原始碼贴出來，方便定位問題。
- 如果涉及到原始碼，提問时請保持原始碼的格式
